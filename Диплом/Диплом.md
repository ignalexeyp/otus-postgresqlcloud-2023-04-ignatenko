...
### с ноды replica
  ...
  if [curl -v vm-postgresql-1:8008/replica 2>&1 | grep '200']; 
  then pg_probackup set-config --instance dev1 --remote-host=vm-postgresql-1  
  elif [curl -v st-postgresql-dev-3:8008/replica 2>&1 | grep '200'];
  then pg_probackup set-config --instance dev1 --remote-host=vm-postgresql-3
  elif [curl -v st-postgresql-dev-1:8008/master 2>&1 | grep '200'];
  then pg_probackup set-config --instance dev1 --remote-host=vm-postgresql-1
  else pg_probackup set-config --instance dev1 --remote-host=vm-postgresql-3
  fi
